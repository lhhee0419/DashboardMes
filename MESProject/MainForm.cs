...
namespace MESProject
{
    public partial class MainForm : Form
    {
        ...
        private void MainForm_Load(object sender, EventArgs e)
        {
            ...
            //string a = WoGrid.SelectedRows[0].Cells[0].Value;
        ...
        private void DataSearch()
        {
            ...
            {
                //배합 콤보박스 선택
                /*                string select_wo_mix = $"SELECT W.WOID, W.PRODID, P.PRODNAME, W.WOSTAT, W.PLANQTY,W.PRODQTY,COUNT(*), W.PLANDTTM, W.ETC "
                                                        +"FROM WORKORDER W, PRODUCT P, LOT L, DEFECTLOT D "
                                                        +"WHERE plandttm >= '{date1.Year}/{date1.Month}/{date1.Day}' and  plandttm <= '{date2.Year}/{date2.Month}/{date2.Day}' AND"
                                                        +"W.PROCID = 'P0001' AND W.PRODID = P.PRODID AND W.WOID = L.WOID AND L.LOTID = D.DEFECT_LOTID GROUP BY W.WOID, W.PRODID, P.PRODNAME, W.WOSTAT, "
            ...
            else if (ProcCombo.SelectedIndex == 1)
            {
                ...
                /*                    $"SELECT W.WOID, W.PRODID, P.PRODNAME, W.WOSTAT, W.PLANQTY,W.PRODQTY,COUNT(*), W.PLANDTTM, W.ETC "
                                                            + "FROM WORKORDER W, PRODUCT P, LOT L, DEFECTLOT D "
                                                            + "WHERE plandttm >= '{date1.Year}/{date1.Month}/{date1.Day}' and  plandttm <= '{date2.Year}/{date2.Month}/{date2.Day}' AND"
                                                            + "W.PROCID = 'P0002' AND W.PRODID = P.PRODID AND W.WOID = L.WOID AND L.LOTID = D.DEFECT_LOTID GROUP BY W.WOID, W.PRODID, P.PRODNAME, W.WOSTAT, "
                                                            + "W.PLANQTY,W.PRODQTY, W.PLANDTTM, W.ETC ";*/
